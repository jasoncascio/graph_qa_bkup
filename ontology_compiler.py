...
class OntologyCompiler:
    ...
    def compile_summary(self) -> str:
        ...
        for prop in self.g.subjects(RDF.type, OWL.ObjectProperty):
            ...
            # Bidirectional Inverse Setup
            inverses = list(self.g.objects(prop, OWL.inverseOf)) + list(self.g.subjects(OWL.inverseOf, prop))
            
            # Agnostic Directive Extraction (with Inverse Inheritance)
            edge_labels = self._get_directive(prop, "gqlEdgeLabel")
            is_inherited_inverse = False
            ...
            # If this edge relies on its inverse for physical mapping, warn the LLM
            if is_inherited_inverse:
...
if __name__ == "__main__":
    ...
    # my_compiler = OntologyCompiler("agentx_people.ttl")
    # print(my_compiler.compile_summary())
    
    # Approach 2: Functional (Legacy)
    compiled_prompt = compile_ontology_for_agent("agentx_people.ttl")
